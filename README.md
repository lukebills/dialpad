...
3. Configure your dictation shortcut: choose **Wispr Flow behaviour** in the editor. **Push to talk** sends **Control + Option** on Mac, with no extra key; physical hold/release behaviour still needs testing. Your existing Mac hands-free shortcuts are **Fn + Space** or **double-tap Fn**, which this editor cannot encode. **Hands-free · custom shortcut** remains available: On Mac, assign **Control + Option + Space** in Flow Settings → General → Shortcuts → Hands-free mode. On Windows the suggested default is **Control + Windows + Space**; verify it in Flow settings. Tap once to start, tap again to stop and paste. Escape cancels and discards a dictation in progress. Dialpad triggers Flow; it does not transcribe audio itself.
